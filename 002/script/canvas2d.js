...
class Canvas2DUtility {
   ...
   * @paran {string} [color] - 矩形を塗りつぶす際の色
   */
  drawRect(x, y, widh, height, color){
    // 色が指定されている場合はスタイルを設定する
    if(color != null){
      this.context2d.fillStyle = color;
    }
    this.context2d.fillRect(x, y, widh, height);
  }
  /**
   * 線分を描画する
   ...
   * @param {number} [width=1] - 線幅
   */
  drawQuadraticBezier(x1, y1, x2, y2, cx, cy, color, widht = 1){
    // 色が指定されている場合はスタイルを設定する
    if(color != null){
      this.context2d.strokeStyle = color;
    }
    // 線幅を設定する
    this.context2d.lineWidth = widht;
    // パスの設定を開始することを明示する
    this.context2d.beginPath();
    // パスの始点を設定する
    this.context2d.moveTo(x1, y1);
